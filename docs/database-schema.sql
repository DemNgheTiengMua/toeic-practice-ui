...
CREATE TABLE Questions (
    ...
    CONSTRAINT FK_Questions_Groups FOREIGN KEY (GroupId) REFERENCES QuestionGroups (GroupId),
...
CREATE TABLE QuestionOptions (
    ...
    CONSTRAINT FK_QOptions_Questions FOREIGN KEY (QuestionId) REFERENCES Questions (QuestionId),
    CONSTRAINT CK_QOptions_Label CHECK (OptionLabel IN ('A', 'B', 'C', 'D'))
);
...
CREATE TABLE ExamAttempts (
    ...
    CONSTRAINT CK_Attempts_Graded CHECK (Status <> 'graded' OR (GradedAt IS NOT NULL AND TotalScore IS NOT NULL)),
...
CREATE TABLE AttemptAnswers (
    ...
    CONSTRAINT FK_AttemptAnswers_Options FOREIGN KEY (SelectedOptionId) REFERENCES QuestionOptions (OptionId)
);
...
CREATE TABLE PracticeAnswers (
    ...
    CONSTRAINT FK_PracticeAnswers_Options FOREIGN KEY (SelectedOptionId) REFERENCES QuestionOptions (OptionId)
);
...
AS
WITH AllAnswers AS (
    -- Câu trong bài thi đã chấm
    SELECT a.UserId, q.PartNumber, aa.IsCorrect
    FROM AttemptAnswers aa
    JOIN ExamAttempts a ON a.AttemptId = aa.AttemptId
    JOIN Questions q ON q.QuestionId = aa.QuestionId
    WHERE a.Status = 'graded' AND aa.IsCorrect IS NOT NULL

    UNION ALL

    -- Câu trong phiên luyện đã xong
    SELECT ps.UserId, q.PartNumber, pa.IsCorrect
    ...
    WHERE ps.Status = 'finished' AND pa.IsCorrect IS NOT NULL
)
SELECT
    aa.UserId,
    aa.PartNumber,
    ...
    SUM(CAST(aa.IsCorrect AS INT)) AS CorrectCount,
    -- Nhân 100.0 để ra số thực, không bị chia nguyên thành 0.
    CAST(100.0 * SUM(CAST(aa.IsCorrect AS INT)) / COUNT(*) AS DECIMAL(5, 2)) AS AccuracyPercent
FROM AllAnswers aa
